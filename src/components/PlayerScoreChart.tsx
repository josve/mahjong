...
const PlayerScoreChart: React.FC<PlayerScoreChartProps> = ({ matches, teamIdToName, allPlayers, teamIdToPlayerIds, playerColors }) => {
  const { playerScores, labels, mahjongWins, highRollerScores, averageHan } = useMemo(() => {
    ...
    Object.keys(totalHan).forEach(player => {
      averageHan[player] = winCount[player] > 0 ? totalHan[player] / winCount[player] : 0;
    });

    return { playerScores: scores, labels, mahjongWins: wins, highRollerScores, averageHan };
  ...
  return (
    <div>
      ...
      <ReactEcharts option={averageHanOptions} style={{ height: "400px" }} />
    </div>
  );
};
